		...
		<div class="flex flex-wrap -mx-1 lg:-mx-4">
			<div v-for="(item, index) in fileList" class="my-1 px-1 w-full md:w-1/2 lg:my-4 lg:px-4 lg:w-1/3">
				<article class="max-w-80 h-60 m-auto flex flex-col overflow-hidden rounded-lg shadow-lg bg-gray-100 dark:bg-gray-700">
					...
					<header class="flex-1 flex flex-col items-center justify-between leading-tight p-2 md:p-4 text-gray-600 dark:text-white">
...
function openVideo(item: FileItem) {
	...
	if(mimeList.includes(item.mime)) window.open(`apps://core.page.api/tools/video/index.html?url=${item.url}&time=${historyItem?.time || 0}`);
