...
watch(fileName, (value) => document.title = `${dirName.value}/${value}`);
...
onMounted(async () => {
	...
	const video_url = search_data.get('url') ?? '';
	const video_time = Number(search_data.get('time') ?? 0);
	url.value = video_url;

	

	emit('url', video_url);
	emit('time', video_time);
