	...
	<div class="w-full h-full py-32 overflow-auto">
		...
		<div class="mx-auto max-w-7xl px-6 lg:px-8">
			<div class="mx-auto max-w-2xl lg:mx-0">
				<h2 class="text-4xl font-bold tracking-tight text-gray-600 dark:text-white sm:text-6xl">RSS订阅源</h2>
				<p class="mt-6 text-lg leading-8 text-gray-600 dark:text-gray-400">您可以在BT站点中添加RSS订阅</p>
			</div>
		</div>
...
const mimeList = ['video/mp4', 'video/ogg', 'video/webm', 'video/quicktime', 'video/x-msvideo', 'video/x-matroska'];

async function getEpisodeInfo() {
	// const dirUrl = options.url.replace(/\/[^\/]+$/, '/');

	const res = await fetch('assets://project.local/.PotionVision/subscribe/', {
		method: 'DIRECTORY',
		headers: {
