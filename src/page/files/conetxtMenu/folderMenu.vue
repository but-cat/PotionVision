...
function openView() {
	// youfo://core.page.api/Code/index.html
	console.log("openView: ", `${root.value}${item.value.path}`, item.value.url);
	
	window.dispatchEvent(new CustomEvent('open-url-page', {
		// detail: `youfo://core.page.api/tools/Assets/index.html$${item.value.url}`,
		detail: `youfo://core.page.api/tools/Assets/index.html$${root.value + item.value.path}`,
	}));
	emit('close');
}
