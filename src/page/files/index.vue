<template>
	<div class="w-full h-full bg-gray-100 dark:bg-gray-900 dark:text-white text-gray-600 flex flex-col overflow-auto relative text-sm">
		<div class="bg-white dark:bg-slate-800 shadow-xl overflow-hidden">
			<div class="w-full h-full overflow-scroll grid grid-cols-[70px,repeat(7,150px)] grid-rows-[auto,repeat(16,50px)]">
				<!-- Calendar frame -->
...
import { defineComponent, reactive, ref, getCurrentInstance, onMounted, IframeHTMLAttributes } from 'vue';

const srcdoc = ref('');
const readEl = ref<HTMLIFrameElement>();

function reload() {
	readEl.value?.contentWindow?.location.reload();
}
...
onMounted(() => {
	...
	// 	srcdoc.value = this.responseText;
	// });
	// oReq.open('GET', (window as any).BASE_URL);
	// oReq.onerror = () => {};
	// oReq.send();
});
</script>
