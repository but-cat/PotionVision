...
export const folderIcons: any[] = [
	...
	{
		name: 'folder-archive',
		folderNames: ['arc', 'arcs', 'archive', 'archives', 'archival', 'bkp', 'bkps', 'bak', 'baks', 'backup', 'backups', 'back-up', 'back-ups', 'history', 'histories'],
	},
...
export const fileNames: Map<string, FolderIconItem> = new Map(folderIcons.map((item) => item?.fileNames?.map((name) => [name, item]) ?? []).flat() as [string, FolderIconItem][]);