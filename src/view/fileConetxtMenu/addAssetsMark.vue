<template>
	<div v-if="open" class="relative z-20" aria-labelledby="modal-title" role="dialog" aria-modal="true">
					...
					<div class="px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
						<div class="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
							...
							<div class="mt-2 w-full">
								...
								<input v-model="url" @click="inputFile?.click()" type="text" autocomplete="address-level1" class="w-full h-36 px-3 py-2 flex items-center justify-between rounded-md bg-white dark:bg-slate-800 border border-slate-400/20 shadow-sm focus:outline outline-2 outline-primary-300/80">
							</div>
						</div>
					</div>
		...
		<input @change="selectFile" type="file" class="hidden" webkitdirectory ref="inputFile"/>
	</div>
</template>
