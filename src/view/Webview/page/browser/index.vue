<template>
	<div class="w-full h-full flex flex-col overflow-hidden">
		<div class="w-full h-14 px-4 py-2 bg-white dark:bg-gray-800 text-slate-600 dark:text-slate-400 relative border-b border-gray-200 dark:border-gray-700 flex items-center shadow-sm">
			<div class="w-48">
        ...
        <div ref="webviewContainer" class="w-full h-full bg-gray-50 dark:bg-gray-900">
...
function setWebViewUrl(val: string) {
	console.log("setWebViewUrl: ", val);
	
	page.value.webview!.loadURL(val);
}


watch(() => page.value.state.src, (val) => {
	console.log("page.value.state.src: ", val);
	
	if (val) contentUrl.value = decodeURIComponent(val);
});

// watch(() => contentUrl.value, (val) => {
// 	console.log("contentUrl: ", val);
	
// 	if (val) page.value.webview!.loadURL(val);
// });
...
onMounted(() => {
	...
	contentUrl.value = decodeURIComponent(page.value.state.src);
});
</script>
