<template>
	<li @click.stop="setActiveItem" @contextmenu.stop="$emit('omnibox', $event)" :class="active ? ['active', 'bg-white', 'dark:bg-gray-800'] : []"
		...
		<!-- <div @error.prevent.stop class="favicon flex justify-center items-center">
			...
			<img v-else="favicon" :src="pageState.favicon"/>
		</div> -->
...
function closeItem() {
	store.commit('page/delWebView', uuid.value);
}
